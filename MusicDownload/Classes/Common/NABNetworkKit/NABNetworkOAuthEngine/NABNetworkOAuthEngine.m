...
@interface NABNetworkOAuthEngine ()

- (NSString *)signatureBaseStringForRequest:(NABNetworkOperation *)request;
...
@implementation NABNetworkOAuthEngine

#pragma mark - Read-only Properties

- (NABNetworkKitOAuthTokenType)tokenType {
    return _tokenType;
}

- (NABNetworkKitOAuthSignatureMethod)signatureMethod {
...
- (id)initWithHostName:(NSString *)hostName customHeaderFields:(NSDictionary *)headers signatureMethod:(NABNetworkKitOAuthSignatureMethod)signatureMethod consumerKey:(NSString *)consumerKey consumerSecret:(NSString *)consumerSecret callbackURL:(NSString *)callbackURL {
...
- (id)initWithHostName:(NSString *)hostName customHeaderFields:(NSDictionary *)headers signatureMethod:(NABNetworkKitOAuthSignatureMethod)signatureMethod consumerKey:(NSString *)consumerKey consumerSecret:(NSString *)consumerSecret {
    return [self initWithHostName:hostName customHeaderFields:headers signatureMethod:signatureMethod consumerKey:consumerKey consumerSecret:consumerSecret callbackURL:nil];
}
...
- (BOOL)isAuthenticated {
    return (_tokenType == NABNetworkKitOAuthAccessToken && self.token && self.tokenSecret);
}

- (void)resetOAuthToken {
    _tokenType = NABNetworkKitOAuthRequestToken;
    _tokenSecret = nil;
    _verifier = nil;
    _customValues = nil;
...
- (void)fillTokenWithResponseBody:(NSString *)body type:(NABNetworkKitOAuthTokenType)tokenType {
    ...
    if (_tokenType == NABNetworkKitOAuthAccessToken) {
...
- (void)setAccessToken:(NSString *)token secret:(NSString *)tokenSecret {
    ...
    _tokenType = NABNetworkKitOAuthAccessToken;
...
- (void)signRequest:(NABNetworkOperation *)request {
    ...
    switch (_signatureMethod) {
        case NABNetworkKitOAuthHMAC_SHA1:
